module RestOfWorldZones
  class CreateJob < ApplicationJob
    def perform(shopify_shipping_zone)
      ...
      @profile_id = @shopify_shipping_zone.profile_id.split('/').last
    ...
    def create_rest_of_world_country_zone
      @rest_of_world_country_zone = find_or_create_spree_zone(
        "Rest of World - Countries/#{@profile_id}",
        'shopify shipping to Countries Rest of World',
        'country'
      )
      create_rest_of_world_state_zone if @existed_spree_zones_by_member_type.key?('Spree::State')
      create_country_members
    end

    def create_rest_of_world_state_zone
      @rest_of_world_state_zone = find_or_create_spree_zone(
        "Rest of World - States/#{@profile_id}",
    ...
    def create_states_members
      Spree::State.
        ...
        where.not(id: @excluded_states_ids).each do |state|
          find_or_create_spree_zone_member('Spree::State', state.id, @rest_of_world_state_zone.id)
        end
    end

    def create_country_members
      Spree::Country.where.not(id: find_excluded_countries_ids).each do |country|
        find_or_create_spree_zone_member('Spree::Country', country.id, @rest_of_world_country_zone.id)
      end
    end
    ...
    def find_excluded_states_ids
      @excluded_states_ids = @existed_spree_zones_by_member_type['Spree::State']&.pluck(:zoneable_id)
    end
    ...
    def find_excluded_whole_countries_ids
      @existed_spree_zones_by_member_type['Spree::Country']&.pluck(:zoneable_id)
    end
